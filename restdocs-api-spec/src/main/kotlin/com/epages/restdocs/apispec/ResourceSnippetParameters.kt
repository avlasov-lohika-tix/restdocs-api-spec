...
data class MultipartRequest(
    val requestParts: Map<String, RequestPartDescriptorWithType> = emptyMap(),
    val requestPartFields: Map<String, List<FieldDescriptor>> = emptyMap(),
    val requestPartsSchemas: Map<String, Schema> = emptyMap()
) : RequestObject()
...
data class ResourceSnippetParameters @JvmOverloads constructor(
    ...
    val request: RequestObject? = null,
...
class RequestPartDescriptorWithType(val name: String) : IgnorableDescriptor<RequestPartDescriptorWithType>() {
    ...
    companion object {
        fun fromRequestPartDescriptor(requestPartDescriptor: RequestPartDescriptor) =
            RequestPartDescriptorWithType(requestPartDescriptor.name)
                .apply {
...
class ResourceSnippetParametersBuilder : ResourceSnippetDetails() {
    ...
    var responseHeaders: List<HeaderDescriptorWithType> = emptyList()
        private set
    var requestObject: RequestObject? = null
        private set
    ...
    fun requestObject(request: RequestObject) = apply { this.requestObject = request }

    fun links(links: List<LinkDescriptor>) = apply { this.links = links }
    fun links(vararg links: LinkDescriptor) = links(links.toList())
