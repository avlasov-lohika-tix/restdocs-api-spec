 ...
 */
internal fun reduceFieldDescriptors(fieldDescriptors: List<FieldDescriptor>): List<FieldDescriptorWithSchemaType> {
...
internal fun unWrapRootArray(jsonFieldPaths: List<JsonFieldPath>, schema: Schema): Schema {
    if (schema is ObjectSchema) {
        val groups = groupFieldsByFirstRemainingPathSegment(Collections.emptyList(), jsonFieldPaths)
...
internal fun toFormattedString(schema: Schema): String {
...
private fun handleEndOfPath(builder: ObjectSchema.Builder, propertyName: String, fieldDescriptor: FieldDescriptorWithSchemaType) {
    ...
    if (fieldDescriptor.ignored) {
        ...
        if (ConstraintResolver.isRequired(fieldDescriptor)) {
    ...
    private fun typeToSchema(type: String): Schema.Builder<*> =
        when (type) {
            ...
            "string" -> StringSchema.builder()
                .minLength(ConstraintResolver.minLengthString(this))
                .maxLength(ConstraintResolver.maxLengthString(this))
